...
const mapping = {
  ...
  '鹤岗': '黑龙江', '双鸭山': '黑龙江', '大庆': '黑龙江', '伊春': '黑龙江',
  '佳木斯': '黑龙江', '七台河': '黑龙江', '牡丹江': '黑龙江',
  '黑河': '黑龙江', '绥化': '黑龙江',
...
const filePath = 'NukemapApp/app/src/main/assets/cities.json';
...
for (const city of cities) {
  if (city.group === '🇨🇳 中国') {
    const province = mapping[city.name];
    if (province) {
      city.group = `🇨🇳 ${province}`;
      updated++;
    } else {
...
console.log(`Updated ${updated} Chinese cities.`);
...
for (const city of cities) {
  if (city.group.startsWith('🇨🇳')) {
    const province = city.group.replace('🇨🇳 ', '');
